import { useParameterValue, useParametersByCategory, useFeatureFlag } from '@/hooks/useParameter';
...
export default function DynamicThemeExample() {
  ...
  const heroTitle = useParameterValue<string>(
    'content.homepage_hero_title',
    'Book Your Perfect Cut'
  );

  // Method 2: Get all parameters from a category
  const { values: themeParams, loading: themeLoading } = useParametersByCategory(
    ParameterCategory.THEME
  );
  const { values: brandingParams, loading: brandingLoading } = useParametersByCategory(
    ParameterCategory.BRANDING
  );
  ...
  const isLoyaltyEnabled = useFeatureFlag('loyalty_program_enabled');
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <Card className="bg-[#0a0a0a] border-stone-800">
          <CardHeader>
            ...
            <CardDescription>
              Dynamically control which features are enabled
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-lg border border-stone-800">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    ...
                    className={
                      isChatEnabled ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10'
                    }
                  >
                ...
                <p className="text-sm text-gray-400">
                  {isChatEnabled
                    ? 'Chat widget is available for customers'
              ...
              <div className="p-4 rounded-lg border border-stone-800">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    variant={isLoyaltyEnabled ? 'default' : 'secondary'}
                    className={
                      isLoyaltyEnabled ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10'
                    }
                  >
        ...
        <Card className="bg-[#0a0a0a] border-stone-800">
          <CardHeader>
            ...
            <CardDescription>
              All theme parameters loaded from the parameter system
            </CardDescription>
          </CardHeader>
          <CardContent>
            {themeLoading ? (
        ...
        <Card className="bg-[#0a0a0a] border-stone-800">
          <CardHeader>
            ...
            <CardDescription>
              All branding parameters loaded from the parameter system
            </CardDescription>
          </CardHeader>
          <CardContent>
            {brandingLoading ? (
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <code className="block p-3 rounded-lg bg-stone-900 text-sm font-mono text-gray-300">
                const primaryColor = useParameterValue&lt;string&gt;('theme.primary_color',
                '#ffffff');
              </code>
            </div>
